...
# Parameters
#
# Update 2025.6.13 to add testing of UDP Port 69 and TCP Ports 8050/8051
                            ...
                            51 {
                                ...
                                53 { 
                                    ...
                                    54 {
                                        ...
                                        $ServerIPs = $list.'Server (Siaddr)'| Select-Object -Unique
                                        ...
                                        function Test-UdpPort {
                                            ...
                                            try {
                                                ...
                                                $IpAddress = [System.Net.Dns]::GetHostAddresses($Server)[0]
                                                
                                                # Create endpoint
                                                $EndPoint = New-Object System.Net.IPEndPoint($IpAddress, $Port)
                                                
                                                # Send test data
                                                $TestData = [Text.Encoding]::ASCII.GetBytes("TEST")
                                                $UdpClient.Send($TestData, $TestData.Length, $EndPoint) | Out-Null
                                                
                                                # Try to receive response
                                                try {
                                                    $RemoteEndPoint = New-Object System.Net.IPEndPoint([System.Net.IPAddress]::Any, 0)
                                                    $ReceivedData = $UdpClient.Receive([ref]$RemoteEndPoint)
                                                    Write-Host "UDP Port $Port is OPEN on $Server" -ForegroundColor Green
                                                    return $true
                                                }
                                                catch {
                                                    Write-Host "UDP Port $Port is OPEN but no response received from $Server" -ForegroundColor Yellow
                                                    return $true
                                                }
                                            }
                                            catch {
                                                Write-Host "UDP Port $Port is CLOSED or FILTERED on $Server" -ForegroundColor Red
                                                return $false
                                            }
                                            finally {
                                                if ($UdpClient) {
                                        ...
                                        foreach ($ServerIP in $ServerIPs){
                                            write-host "Starting Port Tests:" -ForegroundColor Cyan
                                            Test-UdpPort -Server $ServerIP -Port $Port -Timeout $Timeout
                                            
                                            write-host ""
                                            $Result = (Test-NetConnection -ComputerName $ServerIP -Port 8050).TcpTestSucceeded
                                            Write-Host "Test of Port 8050:" -ForegroundColor Gray -NoNewline
                                            if ($Result -eq $True){write-host -ForegroundColor Green "Success"}
                                            else {write-host -ForegroundColor Red "Failed"}
                                            
                                            
                                            $Result = (Test-NetConnection -ComputerName $ServerIP -Port 8051).TcpTestSucceeded
                                            Write-Host "Test of Port 8051:" -ForegroundColor Gray -NoNewline
                                            if ($Result -eq $True){write-host -ForegroundColor Green "Success"}
                                            else {write-host -ForegroundColor Red "Failed"}
                                        }