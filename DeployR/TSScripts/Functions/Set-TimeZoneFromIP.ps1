...
"@
		...
		if ($WindowsPhase -eq 'WinPE') {    
			...
			Get-Volume | Where-Object {$_.DriveType -eq 'Fixed' -and $_.DriveLetter -ne "X"} | ForEach-Object {
				if (Test-Path "$($_.DriveLetter):\Windows\System32\Dism.exe") {
					DISM.EXE /image:$($_.DriveLetter):\ /Set-TimeZone:"$TimeZone"
				}
			}
		}
		else {
