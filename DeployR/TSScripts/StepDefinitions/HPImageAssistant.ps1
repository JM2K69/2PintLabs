...
$updateCategoryFirmware = ${TSEnv:updateCategoryFirmware}
$updateCategoryDrivers = ${TSEnv:updateCategoryDrivers}
$updateCategorySoftware = ${TSEnv:updateCategorySoftware}
$ScanOnly = ${TSEnv:ScanOnly}



Write-Host "=============================================================================="
Write-Host "Current HPIA Settings selected from Task Sequence" 
Write-Host "updateCategoryBIOS: $updateCategoryBIOS"
Write-Host "updateCategoryFirmware: $updateCategoryFirmware"
Write-Host "updateCategoryDrivers: $updateCategoryDrivers"
Write-Host "updateCategorySoftware: $updateCategorySoftware"
Write-Host "ScanOnly: $ScanOnly"

if ($updateCategoryBIOS -eq "true" -or $updateCategoryFirmware -eq "true" -or $updateCategoryDrivers -eq "true" -or $updateCategorySoftware -eq "true"){
    Write-Host "!!Running HPIA with the selected categories!!"
    #Update Variables to Names for Categories
    if ($updateCategoryBIOS -eq "true") {[string]$updateCategoryBIOS = "BIOS"} 
    else {$updateCategoryBIOS = $null}
    if ($updateCategoryFirmware -eq "true") {[string]$updateCategoryFirmware = "Firmware"} 
    else {$updateCategoryFirmware = $null}
    if ($updateCategoryDrivers -eq "true") {[string]$updateCategoryDrivers = "Drivers"} 
    else {$updateCategoryDrivers = $null}
    if ($updateCategorySoftware -eq "true") {[string]$updateCategorySoftware = "Software"} 
    else {$updateCategorySoftware = $null}
    $Category = @($updateCategoryBIOS, $updateCategoryFirmware, $updateCategoryDrivers, $updateCategorySoftware) | Where-Object {$_ -ne $null}
    }
else {
    Write-Host "!!No categories selected, running all updates!!"
    $Category = "All"
...
if ($Category.Count -gt 1){
    Write-Host "Multiple Categories Selected: $($Category -join ",")" -ForegroundColor Yellow
}
else {
    Write-Host "Single Category Selected: $($Category -join ",")" -ForegroundColor Yellow
}
Write-Host "=============================================================================="
...
if ($ScanOnly -eq "true"){
    ...
    Invoke-HPIA -Operation "Analyze" -Category $Category -Selection "All" -Action "List" -NoninteractiveMode
}
else {
